...
- [x] 9. **Docs.** `.env.example`: add `ANTHROPIC_LOGIN_MAIL_SENDERS` (commented default, grammar, invalid entry disables auto-login, "must be EXPORTED") in the claude.ai section. `README.md:216` (`anthropic` row) and the auto-login description: sender allow-list (DMARC `p=reject` rationale), pre-trigger baseline, link must name `ANTHROPIC_LOGIN_EMAIL` (defense in depth), each rejection falls back to assisted login; stated residual risk.
- [x] 10. Lint (`ruff format`, `ruff check`, `mypy`, `pylint` per AGENTS.md), run the Verification block, commit via `ai.py push -m "fix(anthropic): allow-list login-mail senders, skip pre-trigger mails, bind link to account" <files>`.
...
## Deviations (work session 2026-09-24)

- Step 6: a candidate whose body could not be READ (himalaya error) is retried next round instead of being rejected for the attempt; only a readable body without a link is rejected. The per-attempt `rejected` set is a dict (envelope key → reason) so every refusal reason survives the per-round `diag` clear and is printed on failure.
- Step 4: `_himalaya_login_mail_candidates` takes an extra keyword-only `allow` (defaults to `_login_mail_senders()[0]`) so the flow validates the allow-list once.
- Step 5: body reading is factored into `_himalaya_read_message(..., preview)`; `_himalaya_extract_magic_link` wraps it.
- Flow tests live in the new `tests/test_magic_link_flow.py`.
- Incidental: `pytest` added to the dev dependency group — `uv run pytest` used to fall through to a global pytest without playwright (separate commit).
